...
def conduct_council(client, issue_body, system_knowledge):
    ...
    developer_prompt = (
        ...
        f"PLAN: {plan}\n\n"
        "CONSTRAINTS:\n"
        "1. CAUSAL DGP: You MUST generate Y from X (e.g., rbinom(N, 1, plogis(BMI + Age))). Then generate match_status. Then generate paradata using ifelse(match_status == 1, ..., ...).\n"
        "2. CORRUPTION: Corrupt the data using exactly: data$Y[data$match_status == 0] <- sample(data$Y, sum(data$match_status == 0), replace = TRUE).\n"
        "3. SYNTAX: plglm MUST take 'data = data' and 'adjustment = adj_object'. Never pass m.rate to adjMixture.\n"
        "4. Wrap the logic in: test_that('Simulation Sweep', { ... })\n"
        "5. Include 'set.seed(123)' and 'skip_on_cran()' inside the test block.\n"
        "6. Loop through the sensitivity sweep (mismatch = 0.1, 0.3, 0.5).\n"
        "7. NO CHEATING: Do not pass m.rate to adjMixture(). Let the EM estimate it.\n"
        "8. EXTRACT & STORE: Initialize an empty list 'results_list <- list()' before the loop. Inside the loop, extract results using 'tidy_model <- broom::tidy(model_fit)'. Add the mismatch rate: 'tidy_model$mismatch <- mismatch_rates[i]'. Store it: 'results_list[[i]] <- tidy_model'.\n"
        "9. COMBINE: After the loop, do 'final_results <- do.call(rbind, results_list)'.\n"
        "10. OUTPUT: Use `knitr::kable(final_results, format = 'markdown')` to generate the table string, and save it to 'results.md' using exactly: writeLines(table_string, 'results.md'). Do NOT use xtable or append arguments.\n"
